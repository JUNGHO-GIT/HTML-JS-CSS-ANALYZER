...
const parseJSHintConfigJS = (configContent: string): Record<string, any> => {
	try {
		...
		const exportPatterns = cleanContent.match(
			/exports\.(\w+)\s*=\s*([^\n,;}]+)/g,
		);
		exportPatterns &&
		...
		logger(
			`error`,
			`JS config file parsing failed: ${error?.message || error}`,
		);
...
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
const parseJSHintConfigGeneric = (
...
export const loadJSHintConfig = (filePath: string): Record<string, any> => {
	try {
		let baseDir = fs.statSync(filePath).isDirectory()
			? filePath
				...
				fs.existsSync(configPath) &&
					(() => {
						try {
							...
							logger(
								`error`,
								`file parsing error: ${configPath} -> ${parseError?.message || parseError}`,
							);
							return DEFAULT_JSHINT_CONFIG;
						}
					})();
