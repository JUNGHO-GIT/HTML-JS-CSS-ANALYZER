      ...
      for (const configFile of configFiles) {
        ...
        fs.existsSync(configPath) && (() => {
          try {
            ...
            return configFile.endsWith(`.js`) ? parseJSHintConfigJS(configContent) : configFile.endsWith(`.json`) || configFile === `.jshintrc` ? (() => {
              try {
                return JSON.parse(configContent);
              }
              catch {
