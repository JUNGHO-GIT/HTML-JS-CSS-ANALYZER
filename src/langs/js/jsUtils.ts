...
export const calculateSeverity = (error: JSHintError): vscode.DiagnosticSeverity => {
  return !error.code ? vscode.DiagnosticSeverity.Warning : error.code.startsWith(`E`) && ERROR_CODES.has(error.code) ? vscode.DiagnosticSeverity.Error : WARNING_CODES.has(error.code) ? vscode.DiagnosticSeverity.Warning : vscode.DiagnosticSeverity.Information;
};
