...
export const getJSHint = (): JSHintInstance | null => {
	...
	jsHintCache === undefined
		? ((jsHintCache = loadJSHint()), (result = jsHintCache))
		: (result = jsHintCache);
...
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
export const generateAdditionalDiagnostics = (
	...
	analysis.potentialBugs.forEach((bug: PotentialBug) => {
		...
		const severity = ERROR_SEVERITY_TYPES.has(bug.type)
			? vscode.DiagnosticSeverity.Error
			: WARNING_SEVERITY_TYPES.has(bug.type)
				? vscode.DiagnosticSeverity.Warning
				: vscode.DiagnosticSeverity.Information;
	...
	const varUsages = analysis.variables.filter(
		(v: VariableInfo) => v.type === `var`,
	);
	...
	return !jsHint
		...
		: (() => {
				try {
					logger(
						`debug`,
						`starting analysis for: ${document.fileName} (languageId: ${document.languageId})`,
					);
					...
					const { processedCode, analysis } = analyzeSourceCode(
						sourceText,
						document,
					);
					...
					return isValid
						? (logger(
								`debug`,
								`analysis completed: no errors (${document.fileName})`,
							),
							[])
						: (() => {
								const dataMethod =
									(jsHint as any).data || (jsHint.JSHINT as any)?.data;
								const hasDataMethod = typeof dataMethod === `function`;

								return !hasDataMethod
									? (logger(
											`error`,
											`data() method not available in JSHint instance`,
										),
										[])
									: (() => {
											...
											if (result && Array.isArray(result.errors)) {
												...
												logger(
													`debug`,
													`analysis completed: ${errorCount} errors, ${warningCount} warnings, ${infoCount} info (${document.fileName})`,
												);
											}
											const additionalDiagnostics =
												generateAdditionalDiagnostics(document, analysis);
											...
											additionalIssues > 0 &&
												logger(
													`debug`,
													`analysis completed: ${additionalIssues} code quality issues found (${document.fileName})`,
												);
											logger(
												`debug`,
												`analysis finished: total ${totalIssues} issues (${document.fileName})`,
											);
					...
					logger(
						`error`,
						`execution error: ${errorMessage} (${document.fileName})`,
					);
