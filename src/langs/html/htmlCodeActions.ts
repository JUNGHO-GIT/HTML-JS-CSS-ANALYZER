...
const UPPERCASE_TAG_PATTERN = /<\/?([A-Z][\dA-Za-z]*)\b/g;
const UPPERCASE_ATTR_PATTERN = /\s([A-Z][\w-]*)\s*=/g;
...
const VOID_TAG_OPEN_PATTERN = /<([A-Za-z][\dA-Za-z-]*)([^>]*)>/g;
const BAD_CLOSE_PATTERN =
	/<\/(br|hr|img|meta|link|input|source|embed|param|track|area|col|base)\s*>/gi;
...
const createMetaViewportRequireFix: FixFactory = (doc, diagnostic) => {
	...
	const headStart = (head.index as number) + head[0].indexOf(`>`) + 1;
	const metaCharset = head[2].match(
...
const createMetaDescriptionRequireFix: FixFactory = (doc, diagnostic) => {
	...
	const headStart = (head.index as number) + head[0].indexOf(`>`) + 1;
	const metaViewport = head[2].match(
	...
	if (metaViewport) {
		insert =
			headStart + (metaViewport.index as number) + metaViewport[0].length;
	} else if (metaCharset) {
