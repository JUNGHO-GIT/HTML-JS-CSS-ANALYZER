...
export const loadHtmlHint = (): HtmlHintInstance | null => {
	...
	const fnValidate = (mod: unknown): HtmlHintInstance | null => {
		const m = mod as
			...
			| undefined;
		const raw = (m?.default ?? m?.HTMLHint ?? m) as
			| { verify?: unknown }
			| undefined;
		return raw && typeof raw.verify === `function`
			? (raw as HtmlHintInstance)
			: null;
	};
	...
	if (!result) {
		logger(
			`warn`,
			`module not loaded - HTMLHint is optional; install with 'npm install htmlhint' or ensure packaging includes dependency`,
		);
	}
	return result;
};
...
export const loadConfig = (filePath: string): Record<string, unknown> => {
	try {
		let base = fs.statSync(filePath).isDirectory()
			? filePath
