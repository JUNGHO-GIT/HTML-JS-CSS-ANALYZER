...
const addIssue = (
	...
	issues.push({ type, line, column, message, severity, suggestion });
};
...
const analyzeRule = (node: csstree.Rule, issues: CssAnalysisIssue[]): { rules: number; selectors: number } => {
	...
	// 1. Empty Rules
	node.block.children.isEmpty && node.loc && addIssue(
		issues,
		`empty-rule`,
		node.loc.start.line,
		`Empty CSS rule detected`,
		`warning`,
		node.loc.start.column,
		`Remove empty rule or add declarations`
	);
	...
	node.prelude.type === `SelectorList` && node.prelude.children.forEach((selector) => {
		...
		// Too many IDs
		idCount > MAX_ID_SELECTORS && node.loc && addIssue(
			issues,
			`too-many-ids`,
			node.loc.start.line,
			`High specificity: ${idCount} ID selectors in one rule`,
			`warning`,
			node.loc.start.column,
		...
		// Deep nesting
		depth > MAX_SELECTOR_DEPTH && node.loc && addIssue(
			issues,
			`deep-nesting`,
			node.loc.start.line,
			`Selector depth (${depth}) exceeds recommended maximum (${MAX_SELECTOR_DEPTH})`,
			`info`,
			node.loc.start.column,
			`Simplify selector hierarchy`
		);
	});
...
const analyzeDeclaration = (node: csstree.Declaration, issues: CssAnalysisIssue[]): number => {
	...
	// !important usage
	node.important === true && node.loc && addIssue(
		issues,
		`important-usage`,
		node.loc.start.line,
		`Avoid using !important; it breaks cascading`,
		`info`,
		node.loc.start.column,
	...
	// Vendor prefixes
	VENDOR_PREFIX_REGEX.test(node.property) && node.loc && addIssue(
		issues,
		`vendor-prefix`,
		node.loc.start.line,
		`Vendor prefix '${node.property}' detected`,
		`info`,
		node.loc.start.column,
	...
	DEPRECATED_PROPERTIES.has(node.property) && node.loc && addIssue(
		issues,
		`deprecated-property`,
		node.loc.start.line,
		`Property '${node.property}' is deprecated`,
		`warning`,
		node.loc.start.column,
		`Use modern CSS alternatives`
	);
...
const analyzeTypeSelector = (node: csstree.TypeSelector, issues: CssAnalysisIssue[]): void => {
	...
	node.name === `*` && node.loc && addIssue(
		...
		`info`,
		node.loc.start.column,
		`Use specific element or class selectors`
	);
};
...
const analyzeAttributeSelector = (node: csstree.AttributeSelector, issues: CssAnalysisIssue[]): void => {
	...
	isHeavyAttr && node.matcher === null && addIssue(
		issues,
		`universal-selector`,
		node.loc.start.line,
		`Attribute selector [${attrName}] without value can impact performance`,
		`info`,
		node.loc.start.column,
		`Use specific class or ID selectors`
	);
};
...
export const analyzeCssCode = (sourceCode: string): CssAnalysisResult => {
	...
	try {
		...
		error instanceof Error && addIssue(
			issues,
			`syntax-error`,
			1,
...
export const generateCssAnalysisDiagnostics = (
	document: vscode.TextDocument,
	analysis: CssAnalysisResult
): vscode.Diagnostic[] => analysis.issues.map((issue) => {
	...
	const range = new vscode.Range(
		new vscode.Position(safeLineIndex, startCol),
		new vscode.Position(safeLineIndex, endCol)
	);
