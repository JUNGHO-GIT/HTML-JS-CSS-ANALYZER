		...
		CssSupportLike
{
	...
	// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
	private readonly pendingStyles: Map<
		string,
		Promise<Map<string, SelectorPos[]>>
	> = new Map();
	...
	getLocalDoc = async (doc: vscode.TextDocument): Promise<SelectorPos[]> => {
		...
		return cached?.version === ver
			? cached.data
			: (async () => {
					const txt = doc.getText();
					let data: SelectorPos[] = [];
					const isHtml =
						/\.html?$/i.test(doc.fileName) || doc.languageId === `html`;
					if (isHtml) {
	...
	// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
	private readonly fnGetLinkedStyles = async (
			...
			try {
				if (this.isRemoteUrl.test(href)) {
					...
					if (!fs.existsSync(targetPath)) {
						...
						if (
							workspaceFolder &&
							(targetPath.startsWith(path.sep) || targetPath.startsWith(`/`))
						) {
					...
					if (fs.existsSync(targetPath)) {
						try {
							...
							logger(
								`error`,
								`read failed: ${href} -> ${error?.message || error}`,
							);
						}
					}
				}
	...
	getStyles = async (
		...
		const promise = (async () => {
			...
			logger(
				`debug`,
				`collected: ${styleMap.size} entries (workspace files: ${getWorkspaceCssFiles()?.length ?? 0}) for ${doc.fileName}`,
			);
	...
	// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
	private readonly fnGetCompletionItems = async (
		doc: vscode.TextDocument,
		...
		const range = doc.getWordRangeAtPosition(
			position,
			this.wordRange as unknown as RegExp,
		);
		...
		for (const selectors of allStyles.values()) {
			for (const sel of selectors) {
				if (sel.type === kind && !map.has(sel.selector)) {
					...
					item.range = range;
					item.detail =
						kind === SelectorType.ID ? `CSS ID Selector` : `CSS Class Selector`;
	...
	provideCompletionItems = async (
		...
		const isClassContext =
			...
			/classList\.(?:add|remove|toggle|contains|replace)\s*\(\s*["'][^"']*$/.test(
				prefix,
			) ||
			...
			for (const s of selectors) {
				if (s.selector === target) {
					try {
						...
						logger(
							`error`,
							`location parse failed: ${uriString} -> ${errorMessage}`,
						);
					}
				}
			}
