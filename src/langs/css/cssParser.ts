...
const calculateSpecificity = (
	...
	csstree.walk(selector, (node) => {
		...
		(node.type === `ClassSelector` ||
			...
			classes++;
		(node.type === `TypeSelector` || node.type === `PseudoElementSelector`) &&
			node.name !== `*` &&
			elements++;
	});
...
export const parseSelectors = (
	...
	try {
		...
		csstree.walk(ast, (node: csstree.CssNode) => {
			...
			if (
				options?.filterByType !== undefined &&
			...
			const selector =
				typeof node.name === `string` ? node.name.replaceAll(/\\/g, ``) : ``;
			...
			options?.includeSpecificity === true &&
...
export const generateCss = (ast: csstree.CssNode): string =>
