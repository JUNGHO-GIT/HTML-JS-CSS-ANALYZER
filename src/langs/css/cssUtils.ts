...
export const readSelectorsFromFsPath = async (fsPath: string): Promise<SelectorPos[]> => {
  try {
    ...
    return cached?.version === stat.mtimeMs ? cached.data : (async () => {
      return stat.size > MAX_FILE_SIZE ? (logger(`debug`, `file skipped for performance: ${fsPath} (${Math.round((stat.size / 1024 / 1024) * 100) / 100}MB)`), []) : (async () => {
...
const processSingleCssFile = async (filePath: string, styleMap: Map<string, SelectorPos[]>): Promise<void> => {
  return withPerformanceMonitoring(`CSS file processing: ${path.basename(filePath)}`, async () => {
    try {
