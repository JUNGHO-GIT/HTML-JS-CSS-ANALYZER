...
const CLASS_ATTRIBUTE_REGEX =
	/(?<!:)\b(?:class|classname|ngclass)\b\s*=\s*(["'`])((?:(?!\1)[\S\s])*?)\1/gis;
const BOUND_CLASS_ATTRIBUTE_REGEX =
	/\b(?::class|v-bind:class)\b\s*=\s*(["'`])((?:(?!\1)[\S\s])*?)\1/gis;
const ID_ATTRIBUTE_REGEX =
	/(?<!:)\bid\b\s*=\s*(["'`])((?:(?!\1)[\S\s])*?)\1/gis;
const BOUND_ID_ATTRIBUTE_REGEX =
	/\b(?::id|v-bind:id)\b\s*=\s*(["'`])((?:(?!\1)[\S\s])*?)\1/gis;
const TAG_ATTRIBUTE_REGEX =
	/<[A-Za-z][^>]*\b(class|className|ngClass|:class|v-bind:class|id|:id|v-bind:id)\b\s*=\s*(["'`])((?:(?!\2)[\S\s])*?)\2/gi;
const CLASSLIST_METHOD_REGEX =
	/classlist\.(?:add|remove|toggle|contains|replace)\s*\(([^)]+)\)/gis;
const STRING_LITERAL_REGEX = /(["'`])((?:(?!\1).)*?)\1/g;
const QUERYSELECTOR_REGEX =
	/queryselector(?:all)?\s*\(\s*(["'`])((?:(?!\1)[\S\s])*?)\1\s*\)/gis;
const JQUERY_SELECTOR_REGEX =
	/(?:\$|jquery)\s*\(\s*(["'`])((?:(?!\1)[\S\s])*?)\1\s*\)/gis;
const GETELEMENTBYID_REGEX =
	/getelementbyid\s*\(\s*(["'])((?:(?!\1)[^"'`])+?)\1\s*\)/gis;
const GETELEMENTSBYCLASSNAME_REGEX =
	/getelementsbyclassname\s*\(\s*(["'`])((?:(?!\1)[\S\s])*?)\1\s*\)/gis;
const SETATTRIBUTE_REGEX =
	/setattribute\s*\(\s*(["'`])(class|id)\1\s*,\s*([\S\s]*?)\)/gis;
const CLASSNAME_ASSIGN_REGEX =
	/\.classname\s*=\s*(["'`])((?:(?!\1)[\S\s])*?)\1/gis;
...
const fnFetchWithNativeFetch = async (url: string): Promise<string> => {
	...
	!response.ok &&
		(() => {
			const statusInfo =
				response?.statusText ?? `HTTP ${response?.status ?? `unknown`}`;
...
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
const fnFetchWithNodeHttp = async (
	url: string,
				...
				(redirectsRemaining > 0
					? (() => {
							try {
								...
								reject(
									new Error(
										`Invalid redirect location: ${response.headers.location}`,
									),
								);
							}
						})()
...
export const readSelectorsFromFsPath = async (
	fsPath: string,
): Promise<SelectorPos[]> => {
	try {
		...
		return cached?.version === stat.mtimeMs
			? cached.data
			: (async () => {
					return stat.size > MAX_FILE_SIZE
						? (logger(
								`debug`,
								`file skipped for performance: ${fsPath} (${Math.round((stat.size / 1024 / 1024) * 100) / 100}MB)`,
							),
							[])
						: (async () => {
		...
		return err?.code === `ENOMEM` || err?.message?.includes(`out of memory`)
			...
			: (logger(
					`error`,
					`read from file failed: ${fsPath} -> ${err?.message ?? error}`,
				),
				[]);
	}
};
...
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
const processSingleCssFile = async (
	filePath: string,
...
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
export const processCssFilesInBatches = async (
	filePaths: string[],
	styleMap: Map<string, SelectorPos[]>,
): Promise<void> => {
	const uncachedFiles = filePaths.filter(
		(filePath) => !styleMap.has(vscode.Uri.file(filePath).toString()),
	);
	...
	for (let i = 0; i < uncachedFiles.length; i += BATCH_SIZE) {
		...
		const batchPromises = batch.map((filePath) =>
			resourceLimiter().execute(async () =>
				processSingleCssFile(filePath, styleMap),
			),
		);
...
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
export const ensureWorkspaceCssFiles = async (
	...
	try {
		const styleExts = [`css`];
		const configured = getAnalyzableExtensions(folder.uri).filter((e) =>
			styleExts.includes(e),
		);
		...
		for (const glob of patterns) {
			if (collected.length >= MAX_WORKSPACE_FILES) {
				logger(
					`debug`,
					`file limit reached (${MAX_WORKSPACE_FILES} files), remaining files ignored`,
				);
				break;
			}
			...
			for (const uri of uris) {
				...
				!isUriExcludedByGlob(uri, excludePatterns) &&
					collected.push(uri.fsPath);
				collected.length >= MAX_WORKSPACE_FILES &&
					logger(
						`debug`,
						`file limit reached (${MAX_WORKSPACE_FILES} files), remaining files ignored`,
					);
...
export const normalizeToken = (token: string): string => {
	const normalized = !token
		? ``
		: token.replaceAll(TEMPLATE_LITERAL_REGEX, ` `);
	const isQuoted =
		normalized &&
		QUOTE_CHARS.some(
			(quote) => normalized.startsWith(quote) && normalized.endsWith(quote),
		);
...
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
export const collectKnownSelectors = (
	all: Map<string, SelectorPos[]>,
): { knownClasses: Set<string>; knownIds: Set<string> } => {
...
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
export const isValidCssIdentifier = (value: string): boolean =>
	VALID_CSS_IDENTIFIER_REGEX.test(value);
...
export const extractCssBodies = (fullText: string): string => {
	...
	for (let i = 0; i < fullText.length; i++) {
		...
		!inBlockComment &&
			...
			!inLineComment &&
			(ch === `{`
				? (depth === 0 && (start = i + 1), depth++)
				: ch === `}`
					? (depth--,
						depth === 0 &&
							start >= 0 &&
							(bodies.push(fullText.slice(start, i)), (start = -1)))
					: void 0);
	}
...
const collectTokensFromWhitespaceList = (raw: string): string[] => {
	...
	for (const token of tokens) {
		...
		normalizedValue &&
			isValidCssIdentifier(normalizedValue) &&
			out.add(normalizedValue);
	}
	const values = [...out.values()];
	const filtered = raw.includes(`\${`)
		? values.filter((v) => !/[:_-]$/.test(v))
		: values;
	return filtered;
};
...
const collectClassTokens = (raw: string): string[] => {
	...
	const rs = hasExpressionChars
		? collectTokensFromExpression(raw)
		: collectTokensFromWhitespaceList(raw);
	return rs;
};
...
const processClassValue = (
	...
	const tokens = treatAsExpression
		? collectTokensFromExpression(rawClasses)
		: collectClassTokens(rawClasses);
	...
	for (const token of tokens) {
		...
		const highlightStart =
			relativeIdx >= 0 ? baseOffset + relativeIdx : baseOffset;
		...
		knownClasses.has(normalizedValue)
			...
			: (() => {
					const d = new vscode.Diagnostic(
						makeRange(document, highlightStart, highlightLen),
...
const processIdValue = (
	...
	for (const token of tokens) {
		...
		knownIds.has(normalizedValue)
			? usedIds.add(normalizedValue)
			: (() => {
					const d = new vscode.Diagnostic(
						makeRange(document, highlightStart, highlightLen),
						`CSS id '#${normalizedValue}' not found`,
						vscode.DiagnosticSeverity.Warning,
					);
					d.source = `CSS-Analyzer`;
					d.code = `CSS002`;
	...
	while ((literalMatch = localStringLiteralRegex.exec(argumentsString))) {
		...
		knownClasses.has(normalizedValue)
			...
			: (() => {
					...
					const d = new vscode.Diagnostic(
						makeRange(document, tokenStart, literalMatch[2].length),
						`CSS class '${normalizedValue}' not found`,
						vscode.DiagnosticSeverity.Warning,
					);
					d.source = `CSS-Analyzer`;
...
const collectMarkupDefinitions = (
	fullText: string,
	scriptRanges: BlockRange[],
	styleRanges: BlockRange[],
): { markupClasses: Set<string>; markupIds: Set<string> } => {
	...
	while ((m = classRe.exec(fullText))) {
		if (
			isIndexInRanges(m.index, scriptRanges) ||
			isIndexInRanges(m.index, styleRanges)
		) {
			continue;
		}
...
export const scanDocumentUsages = (
	...
	const htmlScriptRanges = isHtml
		? collectHtmlBlockRanges(fullText, `script`)
		: [];
	const htmlStyleRanges = isHtml
		? collectHtmlBlockRanges(fullText, `style`)
		: [];

	// HTML 마크업에 정의된 class/id를 사전수집 → JS 셀렉터에서 참조 시 오탐 방지
	const { markupClasses, markupIds } = isHtml
		? collectMarkupDefinitions(fullText, htmlScriptRanges, htmlStyleRanges)
		: { markupClasses: new Set<string>(), markupIds: new Set<string>() };
	const allKnownClasses =
		markupClasses.size > 0
			? new Set([...knownClasses, ...markupClasses])
			: knownClasses;
	const allKnownIds =
		markupIds.size > 0 ? new Set([...knownIds, ...markupIds]) : knownIds;
	...
	isHtml
		? (() => {
				...
				while ((classAttributeMatch = CLASS_ATTRIBUTE_REGEX.exec(fullText))) {
					if (
						isIndexInRanges(classAttributeMatch.index, htmlScriptRanges) ||
						isIndexInRanges(classAttributeMatch.index, htmlStyleRanges)
					) {
						continue;
					}
					const rawClasses = classAttributeMatch[2];
					const baseOffset =
						classAttributeMatch.index +
						classAttributeMatch[0].indexOf(rawClasses);
					processClassValue(
						rawClasses,
						baseOffset,
						false,
				...
				while ((boundClassMatch = BOUND_CLASS_ATTRIBUTE_REGEX.exec(fullText))) {
					if (
						isIndexInRanges(boundClassMatch.index, htmlScriptRanges) ||
						isIndexInRanges(boundClassMatch.index, htmlStyleRanges)
					) {
						continue;
					}
					const rawClasses = boundClassMatch[2];
					const baseOffset =
						boundClassMatch.index + boundClassMatch[0].indexOf(rawClasses);
					processClassValue(
						rawClasses,
						baseOffset,
						true,
				...
				while ((idAttributeMatch = ID_ATTRIBUTE_REGEX.exec(fullText))) {
					if (
						isIndexInRanges(idAttributeMatch.index, htmlScriptRanges) ||
						isIndexInRanges(idAttributeMatch.index, htmlStyleRanges)
					) {
						continue;
					}
					const rawId = idAttributeMatch[2];
					const baseOffset =
						idAttributeMatch.index + idAttributeMatch[0].indexOf(rawId);
					processIdValue(
						rawId,
						baseOffset,
						false,
				...
				while ((boundIdMatch = BOUND_ID_ATTRIBUTE_REGEX.exec(fullText))) {
					if (
						isIndexInRanges(boundIdMatch.index, htmlScriptRanges) ||
						isIndexInRanges(boundIdMatch.index, htmlStyleRanges)
					) {
						continue;
					}
					const rawId = boundIdMatch[2];
					const baseOffset =
						boundIdMatch.index + boundIdMatch[0].indexOf(rawId);
					processIdValue(
						rawId,
						baseOffset,
						true,
		...
		: (() => {
				...
				while ((tagAttrMatch = TAG_ATTRIBUTE_REGEX.exec(fullText))) {
					...
					(attrName.includes(`class`) || attrName === `ngclass`) &&
						processClassValue(
							raw,
							baseOffset,
							isBound,
	...
	for (const selectorRegex of [QUERYSELECTOR_REGEX, JQUERY_SELECTOR_REGEX]) {
		...
		while ((selectorMatch = selectorRegex.exec(fullText))) {
			...
			while ((m = clsTok.exec(q))) {
				...
				val &&
					(allKnownClasses.has(val)
						? usedClassesFromMarkup.add(val)
						: (() => {
								const start = base + m.index + (m[1] ? 1 : 0) + 1;
								const d = new vscode.Diagnostic(
									makeRange(document, start, val.length + 1),
									`CSS class '${val}' not found`,
									vscode.DiagnosticSeverity.Warning,
								);
								d.source = `CSS-Analyzer`;
								d.code = `CSS001`;
			...
			while ((m = idTok.exec(q))) {
				...
				val &&
					(allKnownIds.has(val)
						? usedIdsFromMarkup.add(val)
						: (() => {
								const start = base + m.index + (m[1] ? 1 : 0) + 1;
								const d = new vscode.Diagnostic(
									makeRange(document, start, val.length + 1),
									`CSS id '#${val}' not found`,
									vscode.DiagnosticSeverity.Warning,
								);
								d.source = `CSS-Analyzer`;
								d.code = `CSS002`;
		...
		id &&
			(allKnownIds.has(id)
				...
				: (() => {
						...
						const d = new vscode.Diagnostic(
							makeRange(document, start, litLen),
							`CSS id '#${id}' not found`,
							vscode.DiagnosticSeverity.Warning,
						);
						d.source = `CSS-Analyzer`;
			...
			val &&
				(allKnownClasses.has(val)
					...
					: (() => {
							...
							const d = new vscode.Diagnostic(
								makeRange(document, start, val.length),
								`CSS class '${val}' not found`,
								vscode.DiagnosticSeverity.Warning,
							);
							d.source = `CSS-Analyzer`;
			...
			attr === `class` &&
				(() => {
					...
					for (const v of classTokens) {
						v &&
							(allKnownClasses.has(v)
								? usedClassesFromMarkup.add(v)
								: (() => {
										const rel = raw.indexOf(v);
										const start = rel >= 0 ? absBase + rel : absBase;
										const d = new vscode.Diagnostic(
											makeRange(document, start, v.length),
											`CSS class '${v}' not found`,
											vscode.DiagnosticSeverity.Warning,
										);
										d.source = `CSS-Analyzer`;
										d.code = `CSS001`;
	...
	for (const s of sels) {
		const used =
			s.type === SelectorType.CLASS
				? usedClasses.has(s.selector)
				: usedIds.has(s.selector);
		!used &&
			(() => {
				...
				const d = new vscode.Diagnostic(
					...
					`Unused CSS selector '${(s.type === SelectorType.CLASS ? `.` : `#`) + s.selector}'`,
					vscode.DiagnosticSeverity.Warning,
				);
				d.source = `CSS-Analyzer`;
				d.code = `CSS003`;
...
export const scanEmbeddedUnused = async (
	...
	for (const s of localDefs) {
		const used =
			s.type === SelectorType.CLASS
				? usedClassesFromMarkup.has(s.selector)
				: usedIdsFromMarkup.has(s.selector);
		!used &&
			(() => {
				const symbolOffset = 1;
				const base = doc.positionAt(s.index);
