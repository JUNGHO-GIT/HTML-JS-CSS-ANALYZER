...
class DiagnosticManager {
	...
	constructor() {
		this.cssCollection =
			vscode.languages.createDiagnosticCollection(`CSS-Analyzer`);
		this.htmlHintCollection =
			vscode.languages.createDiagnosticCollection(`HTMLHint`);
		this.jsHintCollection =
			vscode.languages.createDiagnosticCollection(`JSHint`);
	...
	scheduleValidation(
		...
		triggerMode: AutoValidationMode,
	): void {
		!isAnalyzable(document)
			...
			: (() => {
					...
					const delay =
						newChangeCount >= RAPID_CHANGE_THRESHOLD
							? Math.min(
					...
					try {
						...
						!isForceMode && lastVersion === document.version
							...
							: await (async () => {
									...
									const cssDiagnostics = result.filter(
										(d) => d.source === `CSS-Analyzer`,
									);
									const htmlHintDiagnostics = result.filter(
										(d) => d.source === `HTMLHint`,
									);
									const jsHintDiagnostics = result.filter(
										(d) => d.source === `JSHint`,
									);
									this.cssCollection.set(document.uri, cssDiagnostics);
									this.htmlHintCollection.set(
										document.uri,
										htmlHintDiagnostics,
									);
									...
									logger(
										`debug`,
										`${document.fileName} -> CSS: ${cssDiagnostics.length}, HTML: ${htmlHintDiagnostics.length}, JS: ${jsHintDiagnostics.length}`,
									);
								})();
					} catch (error: any) {
						const errorMessage =
	...
	clearAllCache(): void {
		...
		vscode.window.showInformationMessage(
			`Style cache cleared: ${cacheCountBefore}`,
		);
	}
}
