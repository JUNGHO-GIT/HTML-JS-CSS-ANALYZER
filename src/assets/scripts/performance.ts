	...
	if (!__pmInstance) {
		__pmInstance = {
			...
			start(operationName: string): string {
				...
				if (duration > 500) {
					  logger(`info`, `Performance`, `Slow operation: ${metric.operationName} took ${formattedDuration}ms`);
				}
			...
			checkMemoryUsage(): void {
				...
				if (heapUsedMB > 100) {
					  logger(`info`, `Performance`, `High memory usage: ${heapUsedMB}MB / ${heapTotalMB}MB`);
				}
			},
			cleanup(): void {
				this.metrics.clear();
