		...
		vscode.languages.registerCodeActionsProvider(
			...
			new HtmlHintCodeActionProvider(),
			{
				providedCodeActionKinds:
					HtmlHintCodeActionProvider.metadata.providedCodeActionKinds,
			},
		),
		vscode.languages.registerCodeActionsProvider(
			JS_LANGUAGES,
			...
			{
				providedCodeActionKinds:
					JSHintCodeActionProvider.metadata.providedCodeActionKinds,
			},
		),
	);
};
...
const registerCommands = (
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (mode?: AutoValidationMode) => {
				const editor = vscode.window.activeTextEditor;
				editor &&
					(await updateDiagnostics(
						cssSupport,
						editor.document,
						mode ?? AutoValidationMode.FORCE,
					));
			},
		),
